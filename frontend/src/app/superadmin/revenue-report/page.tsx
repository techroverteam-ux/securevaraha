...
export default function RevenueReport() {
  ...
  const generateDetailExcel = () => {
    ...
    revenueData.forEach(table => {
      ...
      const firstPatient = table.patients[0];
      const scanTypeArray = (firstPatient.scan_type || firstPatient.scanNames?.join(',') || '').split(',').filter(s => s.trim());
      const totalCols = 10 + scanTypeArray.length;
      ...
      for (let i = 1; i <= scanTypeArray.length; i++) {
        htmlContent += `<th style="background-color:#2F75B5; color:white">SCAN-${i}</th>`;
      }
      ...
      table.patients.forEach((patient, index) => {
        ...
        for (let i = 0; i < scanTypeArray.length; i++) {
          htmlContent += `<td>${patientScans[i] || ''}</td>`;
        }
        
        const patientTotalScans = parseInt(String(patient.totalScans || patient.total_scan || 0));
        const patientAmount = parseInt(String(patient.amount || 0));
        
        htmlContent += `
            <td style='text-align:right'>${patientTotalScans}</td>
            <td style='text-align:right'>${patientAmount}</td>
  ...
  const generateSummaryExcel = () => {
    ...
    let htmlContent = `<html><meta http-equiv="Content-Type" content="text/html; charset=Windows-1252"><body><table border="1"><tr><th colspan="13">VARAHA SDC : 256 SLICE CT SCAN</th></tr><tr><th style="text-margin:center;" colspan="13">(IMAGING UNDER P.P.P MODE)</th></tr><tr><th style="text-margin:center;" colspan="13">RAJASTHAN MEDICARE RELIEF SOCIETY, MDM HOSPITAL , Jodhpur</th></tr><tr><th style="background-color:#FFEA00; color:black;text-align:left;" colspan="13">Bill No. :- 2023/VDC_MDM/CT${billNumber}</th></tr><tr><th style="text-align:center;" colspan="13">&nbsp;</th></tr><tr><th style="text-align:right;" colspan="13">RMRS, MDM Hospital, Jodhpur</th></tr><tr><th style="text-align:right;" colspan="13">SUMMARY FOR THE PERIOD OF</th></tr><tr><th style="background-color:#FFEA00; color:black;text-align:right;" colspan="13">${selectedDate}</th></tr>`;
    ...
    revenueData.forEach(table => {
      ...
      htmlContent += `<tr><th style="background-color:#2F75B5; color:white">1. SCAN NAME</th><th style="background-color:#2F75B5; color:white">2. SCAN NAME</th><th style="background-color:#2F75B5; color:white">3. SCAN NAME</th><th style="background-color:#2F75B5; color:white">4. SCAN NAME</th><th style="background-color:#2F75B5; color:white">5. SCAN NAME</th><th style="background-color:#2F75B5; color:white">6. SCAN NAME</th><th style="background-color:#2F75B5; color:white">7. SCAN NAME</th><th style="background-color:#2F75B5; color:white">8. SCAN NAME</th><th style="background-color:#2F75B5; color:white">SCAN NO. ( Scan Code)</th><th style="background-color:#2F75B5; color:white">NO. OF SCAN</th><th style="background-color:#2F75B5; color:white">PATIENT/ FORMS</th><th style="background-color:#2F75B5; color:white">RATE</th><th style="background-color:#2F75B5; color:white">AMOUNT</th></tr>`;
    ...
    // Calculate paid patients (GEN / Paid category) - separate API call needed
    // This should match PHP: SELECT count(*) as tot_patient, sum(total_scan) as tot_scan, sum(amount) as tot_amt 
    // FROM patient_new where scan_date = date and scan_status=1 AND category IN ('GEN / Paid')
    let paidPatients = 0;
    let paidScans = 0;
    let paidAmount = 0;
    
    // For now, calculate from available data - this should be replaced with separate API call
    revenueData.forEach(table => {
      if (table.summaryRows && Array.isArray(table.summaryRows) && table.category === 'GEN / Paid') {
        table.summaryRows.forEach(row => {
          paidPatients += row.patientCount;
          paidScans += row.numberOfScans;
          paidAmount += row.amount;
        });
      }
    });
    
    htmlContent += `<tr><th style="background-color:#FFEA00; color:black; text-align:left" colspan="9">NET AMOUNT</th><th style="background-color:#FFEA00; color:black;text-align:center">${grandTotalScans}</th><th style="background-color:#FFEA00; color:black;text-align:center">${grandTotalForms}</th><th style="background-color:#FFEA00; color:black;text-align:center"> </th><th style="background-color:#FFEA00; color:black;text-align:right">${grandTotalAmount.toLocaleString('en-IN', {minimumFractionDigits: 2, maximumFractionDigits: 2})}</th></tr><tr><th colspan="13">&nbsp;</th></tr><tr><th style="background-color:#2F75B5; color:white" colspan="13">SUMMARY FOR THE PERIOD</th></tr><tr><td><B>PARTICULAR</td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td style="text-align:center"><B>SCAN</td><td></td><td style="text-align:center"><B>AMOUNT</td></tr><tr><td>GROSS TOTAL</td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td style="text-align:center">${grandTotalScans}</td><td></td><td style="text-align:right">${grandTotalAmount.toLocaleString('en-IN', {minimumFractionDigits: 2, maximumFractionDigits: 2})}</td></tr><tr><td>(-) 25% FREE SHARE OF MDM</td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td style="text-align:center">${Math.round(grandTotalScans * 0.25)}</td><td></td><td style="text-align:right">${(grandTotalAmount * 0.25).toLocaleString('en-IN', {minimumFractionDigits: 2, maximumFractionDigits: 2})}</td></tr><tr><th colspan="10" style="background-color:#FFEA00; color:black; text-align:left">NET RECEIVABLE</th><th style="background-color:#FFEA00; color:black; text-align:center">${Math.round(grandTotalScans - (grandTotalScans * 0.25))}</th><th style="background-color:#FFEA00; color:black;"></th><th style="background-color:#FFEA00; color:black;text-align:right">${netReceivable.toLocaleString('en-IN', {minimumFractionDigits: 2, maximumFractionDigits: 2})}</th></tr><tr><th colspan="13">&nbsp;</th></tr><tr><th colspan="13">&nbsp;</th></tr><tr><th colspan="10" style="text-align:left"><u>RUPEES ${numberToWords(netReceivable).toUpperCase()}</u></th><th colspan="3"></th></tr><tr><th colspan="10" style="text-align:left">* Total</th><th colspan="3" style="text-align:right">For : VARAHA SDC</th></tr><tr><th colspan="10" style="text-align:left">*TOTAL PAID PATIENT = ${paidPatients}, TOTAL SCAN = ${paidScans}, TOTAL AMOUNT = ${Math.round(paidAmount)}</th><th colspan="3"></th></tr></table></body></html>`;
          ...
          <div className="overflow-x-auto">
            <Table>
              ...
              <TableBody>
                {loading ? (
                  <TableRow>
                    <TableCell className="text-center" colSpan={13}>Loading...</TableCell>
                  </TableRow>
                ) : !filteredData || filteredData.length === 0 ? (
                  <TableRow>
                    <TableCell className="text-center" colSpan={13}>No revenue data found</TableCell>
                  </TableRow>
                ) : (
                  paginatedRevenue.map((item, index) => (
                    <TableRow key={item.cro || index}>
                      ...
                      <TableCell>{item.scan_names || item.scanNames?.join(', ') || item.scan_type || ''}</TableCell>
                      <TableCell className="text-center">{item.total_scan || item.totalScans || 0}</TableCell>
                      <TableCell className="font-medium text-green-600">₹{item.amount || 0}</TableCell>
                    </TableRow>
                  ))
                )}
              </TableBody>
