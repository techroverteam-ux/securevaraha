...
export default function ConsoleDailyReport() {
  ...
  const fetchReports = async () => {
    ...
    try {
      ...
      const response = await fetch(`https://varahasdc.co.in/api/console/daily-report?${params}`);
      if (response.ok) {
        ...
        const revenue = (data.data || []).reduce((sum: number, report: DailyReport) => {
        ...
        toast.error(`Failed to fetch daily report: ${errorData.error || 'Unknown error'}`);
      }
    } catch (error) {
  ...
  const exportToExcel = () => {
    ...
    try {
        ...
        <html>
          ...
          <body>
            <table>
              ...
              <tr><th colspan="21" class="header">CONSOLE DAILY REPORT - From ${formattedFromDate} To ${formattedToDate}</th></tr>
              <tr>
                <th>S.No</th>
                <th>CRO</th>
                <th>Exam. Id</th>
      ...
      // Calculate totals
      let totalFilms = 0, totalScans = 0, totalCD = 0, totalContrast = 0, totalPaid = 0, totalFree = 0, totalAmount = 0;

      // Add patient data rows
      reports.forEach((report: DailyReport, index: number) => {
        ...
        // Check if free category
        const isFree = ['BPL/POOR', 'Sn. CITIZEN', 'BHAMASHAH', 'RTA', 'JSSY', 'PRISONER', 'IPD FREE', 'OPD FREE', 'RGHS', 'Chiranjeevi', 'Destitute'].includes(report.category);
        ...
        htmlContent += `
          <tr>
            ...
            <td class="center">${isFree ? '' : scans}</td>
            <td class="center">${isFree ? scans : ''}</td>
      ...
      htmlContent += `
              ...
              <tr style="font-weight: bold; background-color: #f0f0f0;">
                ...
                <td class="center">${totalPaid}</td>
                <td class="center">${totalFree}</td>
      ...
      a.href = url;
      a.download = `CONSOLE_DAILY_REPORT_${formattedFromDate}_TO_${formattedToDate}.xls`;
      a.click();
      window.URL.revokeObjectURL(url);
      
      // Update total revenue in UI
    ...
    <div className="p-3 sm:p-6 space-y-4 sm:space-y-6">
      ...
      <div className="bg-gradient-to-r from-sky-500 to-sky-600 text-white p-4 sm:p-6 rounded-xl shadow-lg">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-4 sm:space-y-0">
          <div>
            <h1 className="text-xl sm:text-3xl font-bold mb-2">Daily Report</h1>
            <p className="text-sky-100 text-sm sm:text-base">Console daily activity report</p>
          </div>
      ...
      <div className="bg-white rounded-xl shadow-lg border border-gray-100 overflow-hidden">
        <div className="px-4 sm:px-6 py-4 border-b border-gray-200">
          <h2 className="text-lg sm:text-xl font-semibold text-gray-900">Console Activities</h2>
        ...
        <div className="overflow-x-auto -webkit-overflow-scrolling-touch">
          <table className="w-full min-w-[1200px]">
            <thead className="bg-gray-50">
              <tr>
                <th className="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider whitespace-nowrap">S.No</th>
                <th className="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider whitespace-nowrap">CRO</th>
                <th className="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider whitespace-nowrap">Exam. Id</th>
                <th className="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider whitespace-nowrap">Name</th>
                <th className="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider whitespace-nowrap">Doctor</th>
                <th className="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider whitespace-nowrap">Age</th>
                <th className="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider whitespace-nowrap">Category</th>
                <th className="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider whitespace-nowrap">Scan Type</th>
                <th className="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider whitespace-nowrap">Films</th>
                <th className="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider whitespace-nowrap">No. Scan</th>
                <th className="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider whitespace-nowrap">CD/DVD</th>
                <th className="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider whitespace-nowrap">Contrast</th>
                ...
                <th className="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider whitespace-nowrap">Stop Time</th>
                ...
                <th className="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider whitespace-nowrap">Nursing</th>
                <th className="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider whitespace-nowrap">Remark</th>
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {loading ? (
                ...
                reports.slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage).map((report, index) => (
                  <tr key={`${report.c_p_cro}-${index}`} className="hover:bg-gray-50 transition-colors">
                ...
                {currentPage > 1 && (
                  <>
                    <button
                      ...
                      className="px-2 sm:px-3 py-2 text-xs sm:text-sm font-medium text-gray-500 bg-white border border-gray-300 rounded-lg hover:bg-gray-50"
                    >
                      First
                    </button>
                    <button
